...
var oCONTROLS={ lbl: function(text) { return "<label class='dialog-form-label'>"+text+"</label>"; },
    ...
    //kaip basic + p.text
    txt: function(p) { return this.appendLabel(p, "<input type='text' "+this.basic(p)+((p.Value)?'value="'+$.trim(p.Value)+'" ':'')+"/>"); },
    hidden: function(p) { return this.appendLabel(p, "<input type='hidden' "+this.basic(p)+((p.Value)?'value="'+$.trim(p.Value)+'" ':'')+"/>"); },
    a: function(p) { return "<a "+this.basic(p)+" href='javascript:void(0);return false;'>"+$.trim(p.Value)+"</a>"; },
    ...
    UpdatableForm_toSaved: function(RecId, frm, OtherDataToSave) {//Darasom, value kiekvienam
        if(OtherDataToSave) { oGLOBAL.UpdateServer({ Action: "Edit", DataToSave: OtherDataToSave }); }
    ...
    UpdatableForm: function(frm) {
        ...
        $(frm).find('div.ExtendIt, span.ExtendIt').each(function() {
            ...
            var col={}, input; if(data!=="NoData") { col=$.extend({}, data.Cols[ix]); } //Naujas objektas turi but
            col=$.extend({}, col, eOpt); //overridinu data.Cols<----e.data('ctrl')
            var Type=(col.Type)?col.Type:((col.List)?"List":""); // (col.List)?"List":col.Type;
            if(Type===undefined) { alert("Nesusiparsino ctrl elemento objFunc.js-UpdatableForm (nėra Type)"); return true; }
            ...
            for(var prop in col) {
                if(prop==='List') { $.extend(data_ctrl, col[prop]); } //Listo propercius dedu vienam lygyje su kitais data_ctrl.Type="List";
            ...
            //if(Type==='List') {
            //else
            if(Type==='Boolean'||Type==='checkbox') { eHTML=oCONTROLS.chk(col); $(eHTML).prependTo(e); }
                ...
                eHTML+=oCONTROLS.txt(col);
                if(isTime) {
                    eHTML+=oCONTROLS.txt({ Value: TimeValue, style: "margin-left:20px;", title: "Laikas", classes: "time ui-widget-content ui-corner-all" });
                } // UpdateField nereikia, nes..
                input=$(eHTML).prependTo(e).parent().find('input:first');
                input.ValidateOnBlur({ Allow: 'date' });
            }
            if(col.List) {
                if(input) { input.ComboBox(); } else { eHTML+=oCONTROLS.txt(col); input=$(eHTML).prependTo(e).parent().find('input').ComboBox(); }
            }
            if(col.Plugin) {
...
var oGLOBAL={
    ...
    GetNo: function(i) {
        ...
        DefMsg={ Title: "Duomenų keitimas.",
            Error: { Add: "Nepavyko išsaugot naujų duomenų.", Edit: "Nepavyko pakeisti duomenų.", Delete: "Nepavyko ištrinti duomenų." },
        ...
        if(resp.ErrorMsg) {
            ...
            Msg+=" Klaida:\n"+resp.ErrorMsg;
            if(updData.CallBack) { if(typeof updData.CallBack.Error==='function') { updData.CallBack.Error(resp); } }
            DIALOG.Alert(Msg, DefMsg.Title); return false;
        } else {
            Msg=MsgObj.Success[updData.Action];
            if(updData.CallBack) { if(typeof updData.CallBack.Success==='function') { updData.CallBack.Success(resp, updData); } }
            $.growlUI(DefMsg.Title, Msg); return false;
        }
            ...
            if(e.attr("type")==="checkbox") {
                ...
                if(typeof e.data("ctrl").Type==='undefined') {
                    ...
                    if(Type==="hidden") {
                        ...
                        val=e.val(); eval("$(this)."+Validity);
                        if(Type.search(/time/i)> -1) {
                            var TimeCtrl=e.parent().next()
                            TimeCtrl.require().match(/^\d{2}[:]\d{2}$/, "Turi būt laiko formatas - valandos:minutės, pvz.12:30");
